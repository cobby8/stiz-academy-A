# 최근 변경 추가
- 2026-07-15: 학생 상세 화면에서 수강 반별 `수강 중/휴원/퇴원` 상태를 직접 변경할 수 있게 했다. 서버 액션은 허용 상태값만 받도록 검증하고, 변경 후 학생/반/수납/통계 캐시를 무효화한다.
- 2026-07-15: 관리자 수납/결제 화면에 시트 원장 기준 수납 점검·적용 흐름을 추가했다. 최신 완료 이관 배치 기준으로 수납 기록 생성/수정/동일/확인 필요를 분리하며, DB 미리보기 검증 결과 생성 7건/수정 72건/확인 필요 31건이 계산됐다.
- 2026-07-14: 관리자 원생 관리에 7월 최신 원생목록 점검 패널을 추가했다. 새 읽기 전용 API는 최신 완료 이관 배치 기준으로 7월 등록 행의 연결률, 이전 월 이력, 중복 의심 Student, 이름 충돌, 시간표 미매칭을 계산한다.
- 2026-07-14: Google Form 2개를 기준으로 자체 체험수업/수강신청 폼의 입력 항목을 맞추고, 신규 항목을 DB 저장 컬럼과 관리자 신청 상세 표시까지 연결했다.
- 2026-07-14: 대표팀 미연결 원본 중 생년월일 확인 가능한 8건을 실제 `Student`로 생성하고 `StudentTeamRosterEntry`/`StudentSheetRawRow`와 연결했다. 총 대표팀 25건 중 연결 19건/미연결 6건이며, 연결 완료된 행의 오래된 영어 경고 이슈는 정리하고 실제 보류 6건만 남겼다.
- 2026-07-13: 대표팀 원본은 기존 DB 학생과 매칭되지 않을 때 관리자 화면에서 `새 학생 생성`으로 처리할 수 있다. 생년월일을 해석할 수 있는 행만 버튼을 보여주며, 보호자 정보는 `확인 필요` 상태의 임시 보호자 계정/학생 메모로 남긴 뒤 원본 행과 즉시 연결한다.
- 2026-07-13: 미연결 원본 점검에서 이름/전화/생년월일/학년/학교 같은 연결 단서가 없는 행은 `검토 제외`로 분리한다. 최신 배치 기준 차량 미연결 994건 중 992건은 빈 서식성 행이라 수동 후보에서 제외하고, 실제 검토 후보는 차량 2건과 대표팀 14건으로 줄였다.
- 2026-07-13: 관리자 수강생 화면에 차량·대표팀 원본 학생 연결 점검 패널을 추가했다. relink API는 검토 목록(`reviewRows`)과 개별 수동 연결(`action: manual`)을 지원하며, 확정 후보는 일괄 적용하고 이름 후보/후보 없음 행은 관리자가 학생을 선택해 한 줄씩 연결할 수 있다.
- 2026-07-13: 실제 Supabase DB에 `ScheduleSlot`, `ScheduleImportBatch`, `ScheduleImportIssue`를 적용하고 기존 시트 캐시/수동 슬롯 32건을 `ScheduleSlot`으로 이관했다. 최신 시간표 이관 배치 `cd03bfa2-b204-4dc4-8690-0a39d252c093`는 `IMPORTED`, 오류 0건/경고 21건이며, `Class`는 31건 갱신/1건 생성됐다.
- 2026-07-13: 월별 시트에 반복 등장하는 수강생은 아이 이름만으로 확정하지 않고, 아이 이름+학부모 연락처/학생 연락처/생년월일을 우선 매칭하는 공통 유틸과 relink API를 추가했다. 최신 이관 배치에서 차량 1410행은 강한 연락처 매칭으로 기존 학생과 재연결했고, 차량 994행/대표팀 13행과 이름 단독 약한 후보 1행은 수동 확인 대상으로 남겼다.
- 2026-07-13: 최신 시트 원장을 기준으로 `Enrollment` 누락 생성/비활성 전환 대상을 미리보고 적용할 수 있는 수강 등록 reconciliation API와 관리자 버튼을 추가했다.
- 2026-07-13: 실제 수강생 운영 시트를 DB에 이관하고, 최신 이관 요약/시간표 차이/차량·변동·대표팀·이슈 상세 지연 조회와 수동 이관 후 관리자 캐시 무효화를 추가했다.
- 2026-07-13: 2026 다산점 가입신청서의 `등록`, 월별 차량, `변동내역메모`, `대표팀 명단` 데이터를 모두 DB에 보존할 수 있도록 수강생 시트 이관 배치/원본 행/등록 원장/차량/변동/대표팀/이슈 모델과 운영 SQL을 추가했다.
