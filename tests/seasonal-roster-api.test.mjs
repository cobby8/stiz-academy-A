...
test("시즌 반 날짜 요일 결제 셔틀 검색 필터를 서버에서 처리한다", () => {
  for (const name of ["seasonId", "offeringId", "weekday", "date", "paymentStatus", "shuttleStatus", "q"]) {
    assert.match(route, new RegExp(`params\\.get\\("${name}"\\)`));
  }
  assert.match(route, /AT TIME ZONE 'Asia\/Seoul'/);
  assert.match(route, /INVALID_ROSTER_DATE/);
});
...
test("CSV helper가 이후 같은 명단 DTO를 소비할 수 있게 조회와 응답 조립이 분리돼 있다", () => {
  ...
  assert.match(route, /filters: \{ seasonId, offeringId, weekday, date: rosterDate, paymentStatus, shuttleStatus, q: query \}/);
});
