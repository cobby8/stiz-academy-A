  ...
  return `
    ...
    targets AS (
      SELECT
        *,
        CASE
          WHEN "targetEnrollmentStatus" <> 'ACTIVE' THEN NULL
        ...
        CASE
          ...
          WHEN "paidAmount" = 0 AND "pendingAmount" = 0 AND "hasReviewOnlyMethod" THEN '추가수강/이월 행만 있어 수동 확인이 필요합니다.'
    ...
    actions AS (
      SELECT
        ...
        CASE
          WHEN t."targetStatus" IS NULL OR COALESCE(t."targetAmount", 0) <= 0 THEN 'REVIEW'
          WHEN e.id IS NULL THEN 'CREATE'
