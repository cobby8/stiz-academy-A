    ...
    return (
        <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
            <div className="flex items-center gap-4">
                <h2 className="font-extrabold text-gray-900">학원 홈페이지 캔버스 빌더</h2>
                <div className="flex bg-gray-100 rounded p-1">
                    <button
                        className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-1 ${enabled ? 'bg-white shadow text-brand-orange-600' : 'text-gray-500'}`}
                        onClick={() => actions.setOptions((options) => (options.enabled = true))}
                    >
                        <Edit3 className="w-3.5 h-3.5" /> 편집 모드
                    </button>
                    <button
                        className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-1 ${!enabled ? 'bg-white shadow text-brand-navy-900' : 'text-gray-500'}`}
                        onClick={() => {
                            actions.selectNode("");
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex bg-gray-100 rounded p-1 mr-2">
                    <button
                        className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-1 ${deviceWidth === 'mobile' ? 'bg-white shadow text-gray-900' : 'text-gray-500'}`}
                    ...
                    <button
                        className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-1 ${deviceWidth === 'pc' ? 'bg-white shadow text-gray-900' : 'text-gray-500'}`}
                ...
                <button
                    onClick={saveDesign}
                    className="bg-brand-orange-500 text-white px-5 py-2 rounded text-sm font-bold shadow hover:bg-brand-orange-600 transition flex items-center gap-2"
                >
