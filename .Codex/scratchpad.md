...
## 현재 작업
- 작업명: 방학특강 관리자 신청 상세 개선
- 상태: 구현 및 검증 완료, 커밋 대기
- 범위: 신청 상세 정보 표시, 셔틀 요청 표시, 항목별 빠른 처리, 회귀 테스트
- 기준일: 2026-07-20

## 진행 현황표
| 항목 | 상태 | 메모 |
| --- | --- | --- |
| 신청 상세 정보 | 완료 | 생년월일, 연락처, 주소, 접수일, 예상 금액 표시 |
| 셔틀 요청 | 완료 | 탑승 위치, 희망 시간, 하차 위치, 요청 메모 표시 |
| 빠른 처리 | 완료 | 승인, 대기, 반려, 취소 버튼과 직접 상태 변경 유지 |
| 회귀 테스트 | 완료 | 관리자 UX 정적 테스트 추가 |
...
- `tests/seasonal-admin-ux.test.mjs`: 셔틀 상세와 빠른 처리 UI가 빠지지 않도록 회귀 테스트 추가.

## 테스트 결과
- `cmd /c node_modules\.bin\tsc.cmd --noEmit`: 통과
- `cmd /c node_modules\.bin\eslint.cmd src\app\admin\seasonal\SeasonalAdminClient.tsx tests\seasonal-admin-ux.test.mjs`: 통과
