# STIZ 작업 스크래치패드
## 현재 작업
- 목표: 운영·학부모 소통 담당과 보완 협업. G1 예약 설정 백업 → G2 상담 후속 처리 → G3 입학 증거 연결. 월 장부 후속 과제는 유지한다.
- 현재 단계: 실제 UI·권한·API·격리 DB 통합20·DB/복원37·브라우저19·회귀87·tsc·lint·QA 통과, 임시 서버 종료 확인. 실제 Supabase 인증·전체 운영 통합·시트 대체 미완료, 저장 기본 off.
