                                ...
                                <div key={month}>
                                    ...
                                    <div className="space-y-3">
                                        {eventsByMonth[month].map((event) => {
                                            ...
                                            const hasDetail = !!(event.description || event.url);
                                            return (
                                                <button
                                                    key={event.id}
                                                    onClick={() => setSelectedEvent(event)}
                                                    className={`w-full text-left ${catStyle.bg} rounded-xl p-4 border ${catStyle.border} flex flex-col sm:flex-row sm:items-start gap-3 transition hover:brightness-95 active:scale-[0.99] cursor-pointer group`}
                                                >
                                                    {/* 날짜 영역 */}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex flex-wrap items-center gap-2 mb-1">
                                                            <span className={`flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 rounded-full border ${catStyle.bg} ${catStyle.text} ${catStyle.border}`}>
                                                    ...
                                                    {/* 상세보기 화살표 */}
                                                    <div className="shrink-0 self-center text-gray-300 group-hover:text-gray-500 transition text-lg">
                                                        ›
                                                    </div>
                                                </button>
                                            );
