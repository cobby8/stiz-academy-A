...
test("체험 신청 카드형은 목록형과 같은 핵심 항목만 단순하게 배치한다", () => {
  ...
  assert.match(trialClient, /접수일[\s\S]*희망일[\s\S]*수업교시[\s\S]*확정일정/);
...
test("목록형은 스프레드시트처럼 실제 테이블 구조로 보여준다", () => {
  assert.match(trialClient, /function renderTrialList\(\)[\s\S]*<table className="w-full min-w-\[1180px\]/);
  assert.match(trialClient, /<thead[\s\S]*접수일[\s\S]*희망일[\s\S]*수업교시[\s\S]*확정일정[\s\S]*쌤알림/);
