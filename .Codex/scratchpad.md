...
## 2026-07-15 선생님 모바일 수업 관리 작업 로그
- 선생님 계정과 Coach·시간표·담당 수업·학생을 연결하는 공통 서버 권한 검사를 추가하고 `tsc --noEmit` 검증을 통과했다.
- 수업의 계획·진행·완료 상태, 실제 시작·종료 시각, 사전 수업 내용과 중복 방지 키를 기존 기록 보존 방식으로 설계했다.
- 담당 수업 권한을 재확인하고 중복 클릭 시 최초 시작 시각을 유지하는 수업 시작 Server Action을 추가했다.
- 관리자 수납/결제에 시트 원장 기준 수납 점검·적용 흐름을 추가했다. 검증: `npx.cmd tsc --noEmit`, focused ESLint, DB 미리보기 쿼리 통과.
- 학생 상세 화면에서 수강 반별 `수강 중/휴원/퇴원` 상태를 직접 변경할 수 있게 했다. 검증: `npx.cmd tsc --noEmit`, 학생 상세 focused ESLint, DB 상태 분포 조회 통과.

## 2026-07-14 작업 로그
- 체험 CRM과 수강신청 관리를 `/admin/apply` 통합 탭으로 묶고, 체험완료 상태 전환 시 학부모에게 감사 안내 및 수강신청 링크 SMS가 1회 자동 발송되도록 구현했다. 검증: `npx.cmd tsc --noEmit` 통과.
- 2026-07-15: `/admin/apply` 안내 설정 탭에서 더 이상 사용하지 않는 신청 폼 모드 설정 카드를 제거했다. 검증: `npx.cmd tsc --noEmit` 통과.
- 2026-07-15: 체험완료 리드에 유선상담 후 수강신청/입학 안내 문자 발송 버튼을 추가하고, 수강신청 제출 시 이름+학부모 연락처로 체험 리드를 자동 매칭해 등록 전환 흐름을 추적하도록 연결했다. 검증: `npx.cmd tsc --noEmit` 통과.
- 2026-07-15: 체험 CRM 카드에 담당 선생님 알림 버튼을 추가하고, 시간표 담당 코치에게 체험수업 정보 문자를 즉시 발송하며 발송 이력을 카드에 표시하도록 구현했다. 검증: `npx.cmd tsc --noEmit` 통과.

## 현재 작업
- 작업명: 학생 상태 실시간 웹 관리 1단계
- 상태: 학생 상세 수강 상태 변경 UI와 서버 검증 구현 완료
- 범위: 수강 반별 ACTIVE/PAUSED/WITHDRAWN 직접 변경, 관련 캐시 무효화
- 기준일: 2026-07-15
...
| 수납 정합성 점검 | 완료 | 최신 원장 기준 생성/수정/확인 필요를 분리하고 관리자 수납 화면에서 수동 적용 |
| 학생 상태 웹 관리 | 완료 | 학생 상세에서 수강 반별 수강 중/휴원/퇴원 상태를 직접 변경 |
| 배포/검증/의존 제거 | 진행 중 | 대표팀 미연결 14건 중 8건 실제 학생 생성/연결 완료, 6건만 원본 확인 필요 |

## 작업 로그
- 2026-07-15: 학생 상세 수강 정보 카드에 상태 스위치를 추가하고, 서버 액션에서 허용 상태 검증과 학생/반/수납 캐시 무효화를 연결했다.
- 2026-07-15: 관리자 수납/결제 화면에 시트 원장 기준 점검과 적용 버튼을 추가하고, 최신 배치 기준 생성 7건/수정 72건/확인 필요 31건을 미리보기 쿼리로 검증했다.
