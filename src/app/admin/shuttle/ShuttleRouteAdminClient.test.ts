...
test("운행일별 노선과 미배정 후보를 분리한다", () => {
  ...
  assert.match(source, /shuttle-service-dates/);
...
test("관리자는 기사 체크 현황을 즉시 또는 자동으로 새로고침한다", () => {
  ...
  assert.match(source, /SHUTTLE_AUTO_REFRESH_MS = 60_000/);
  assert.match(source, /window\.setInterval/);
  assert.match(source, /새 상태 불러오기/);
  assert.match(source, /1분 자동 새로고침/);
});
...
test("모바일에서도 순서 변경은 가능하고 관리자 길안내 링크는 노출하지 않는다", () => {
  assert.match(source, /순서를 위로 이동/);
  assert.match(source, /순서를 아래로 이동/);
  assert.doesNotMatch(source, /https:\/\/map\.kakao\.com\/link\/map/);
