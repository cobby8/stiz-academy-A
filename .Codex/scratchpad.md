...
| 항목 | 상태 | 메모 |
| --- | --- | --- |
| 원인 확인 | 완료 | 문준현 row는 확정 반 금6, 기존 희망 슬롯 수6이 함께 저장됨 |
| 표시 로직 | 완료 | 목록 수업 컬럼은 scheduledClassId 확정 반을 먼저 표시 |
| 회귀 방지 | 완료 | 신청 관리 UX 테스트에 확정 반 우선 표시 규칙 추가 |
| 검증 | 완료 | TypeScript, ESLint, 신청 관리 UX 테스트 통과 |

## 구현 기록
- `src/app/admin/trial/TrialCrmClient.tsx`: 체험신청 목록 수업 컬럼에서 확정 수업 반을 희망 슬롯보다 우선 표시.
- `tests/application-management-ux.test.mjs`: 확정 반 우선 표시 로직 회귀 테스트 추가.
