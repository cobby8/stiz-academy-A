...
export default function RouteSection({ initial, date, refreshKey, apiBase = "/api/admin/seasonal/dispatch", rosterEditable = true }: { initial: DispatchSuggestion; date: string; refreshKey: number; apiBase?: string; rosterEditable?: boolean }) {
  ...
  return (
    <section className="rounded-[6px] border border-[var(--doc-rule)] bg-[var(--doc-surface)] p-3">
      {/* 섹션 헤더: 시간 · 방향 */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--doc-rule)] pb-2">
        <div className="flex items-center gap-2">
          <span className={`grid h-8 w-8 place-items-center rounded-[3px] text-lg ${isPickup ? "bg-[var(--doc-grid-head)] text-[var(--doc-ink-2)]  " : "bg-[var(--doc-grid-head)] text-[var(--doc-warn)] "}`}>{isPickup ? "등원" : "하원"}</span>
          <div>
            <p className="text-[15px] font-bold text-[var(--doc-ink)]">{sectionTime ?? "-"} · {isPickup ? "등원" : "하원"}</p>
            <p className="text-[11.5px] font-bold text-[var(--doc-ink-2)]">{sug.classStart ?? "-"}{sug.classEnd ? `~${sug.classEnd}` : ""} 수업 · 탑승 {sug.totalRiders}명{rerouting ? " · 경로 재계산 중" : ""}</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 print:hidden">
          <button onClick={() => { setLoadedFromSaved(false); setSaveMsg(null); void generate(date); }} disabled={loading} className="rounded-[3px] bg-[var(--doc-ink)] px-2.5 py-1.5 text-[12px] font-bold text-white disabled:opacity-50 dark:text-[var(--doc-ink)]">{loading ? "계산 중…" : "자동 제안"}</button>
          <button onClick={saveRoute} disabled={saving || sug.vehicles.length === 0} className="rounded-[3px] bg-[var(--doc-accent)] px-2.5 py-1.5 text-[12px] font-bold text-white disabled:opacity-50">{saving ? "저장 중…" : "저장"}</button>
          {loadedFromSaved && <button onClick={() => void recalcPaths()} disabled={loading || rerouting || recalcing} className="rounded-[3px] border border-[var(--doc-rule)] px-2.5 py-1.5 text-[12px] font-bold text-[var(--doc-ink-2)] disabled:opacity-50">{recalcing ? "계산 중…" : "경로 재계산"}</button>}
          {loadedFromSaved && <button onClick={deleteRoute} disabled={saving} className="rounded-[3px] border border-[var(--doc-crit)] px-2.5 py-1.5 text-[12px] font-bold text-[var(--doc-crit)] disabled:opacity-50">노선 삭제</button>}
        </div>
      </div>

      {err && <p className="mt-2 rounded-[3px] bg-[var(--doc-crit-soft)] px-3 py-2 text-xs font-bold text-[var(--doc-crit)]">{err}</p>}
      {loadedFromSaved && <p className="mt-2 rounded-[3px] bg-[var(--doc-grid-head)] px-3 py-2 text-[11.5px] font-bold text-[var(--doc-ink-2)]">저장된 노선{savedAt ? ` · ${fmtSaved(savedAt)} 저장` : ""} · 「자동 제안」으로 새로 계산</p>}
      {saveMsg && <p className="mt-2 rounded-[3px] bg-[var(--doc-accent-soft)] px-3 py-2 text-[11.5px] font-bold text-[var(--doc-accent)]">{saveMsg}</p>}

      {/* 위치 미세조정 자동 반영 안내 — 좌표가 바뀐 학생의 경로만 자동 갱신됐다. 배정·순서는 그대로. */}
      {relocatedCount > 0 && (
        <p className="mt-2 rounded-[3px] bg-[var(--doc-grid-head)] px-3 py-2 text-[11.5px] font-bold text-[var(--doc-ink-2)] print:hidden">
      ...
      {(sug.added ?? []).length > 0 && (
        <div className="mt-2 rounded-[3px] border border-[var(--doc-warn)] bg-[var(--doc-grid-head)] px-3 py-2.5 print:hidden">
          <p className="text-[12px] font-bold text-[var(--doc-warn)]">신규·복귀 {(sug.added ?? []).length}명 — 어디에 태울지 정해 주세요(추천은 미리 골라 뒀습니다).</p>
          <ul className="mt-1.5 space-y-1.5">
            {(sug.added ?? []).map((c) => {
              ...
              return (
                <li key={c.requestId} className="flex flex-wrap items-center gap-1.5 rounded-[3px] bg-[var(--doc-surface)] px-2.5 py-1.5">
                  <span className="text-[12.5px] font-bold text-[var(--doc-ink)]">{c.name}</span>
                  <span className="text-[11px] font-bold text-[var(--doc-ink-2)]">{c.isHub ? "🆓 무료탑승" : (c.label || "위치")}</span>
                  {noCoord
                    ? <span className="ml-auto text-[11px] font-bold text-[var(--doc-crit)]">좌표 없음 · 무료탑승만 가능</span>
                    : (
                      <span className="ml-auto flex items-center gap-1">
                        {!c.isHub && sug.vehicles.length > 1 && (
                          <select value={chosen} onChange={(e) => setAssignVi((m) => ({ ...m, [c.requestId]: Number(e.target.value) }))}
                            className="rounded-[3px] border border-[var(--doc-rule)] px-1.5 py-1 text-[11px] font-bold">
                        ...
                        <button onClick={() => assignAdded(c, c.isHub ? rec : chosen)} disabled={hubBusy}
                          className="rounded-[3px] bg-[var(--doc-accent)] px-2.5 py-1 text-[11px] font-bold text-white disabled:opacity-50">
      ...
      {sug.vehicles.length === 0 && <div className="mt-3 rounded-[3px] border border-dashed border-[var(--doc-rule)] p-6 text-center text-sm text-[var(--doc-ink-3)]">이 날짜에 배차할 {isPickup ? "등원" : "하원"} 셔틀 학생이 없습니다.</div>}

      <div className="mt-3 lg:grid lg:grid-cols-2 lg:items-start lg:gap-4">
        <div className="grid min-w-0 gap-3">
          {sug.vehicles.map((v, vIdx) => (
            <div key={vIdx} className="overflow-hidden rounded-[6px] border border-[var(--doc-rule)]">
              <div className="flex items-center justify-between px-4 py-2.5" style={{ background: "var(--doc-grid-head)", color: "var(--doc-ink)", borderBottom: "1.5px solid var(--doc-ink)" }}>
                <span className="font-bold">{v.vehicleName}{v.tripLabel ? ` · ${v.tripLabel}` : ""}</span>
                <span className="flex items-center gap-2">
                  <button type="button" onClick={() => setMapVehicle(vIdx)} className="no-print rounded-[3px] px-2.5 py-0.5 text-xs font-bold" style={{ border: "1px solid var(--doc-rule-strong)", color: "var(--doc-ink-2)" }}>지도</button>
                  <span className="rounded-[3px] px-2.5 py-0.5 text-xs font-semibold tabular-nums" style={{ border: `1px solid ${v.over ? "var(--doc-crit)" : "var(--doc-rule-strong)"}`, color: v.over ? "var(--doc-crit)" : "var(--doc-ink-2)", background: v.over ? "var(--doc-crit-soft)" : "transparent" }}>{v.passengers} / {v.capacity}명</span>
                </span>
              </div>
              <div className="flex items-center gap-2 border-b border-[var(--doc-rule)] bg-[var(--doc-grid-head)] px-3 py-1.5 text-[11px] font-bold">
                {v.provider === "TMAP"
                  ? <span className="rounded bg-[var(--doc-grid-head)] px-1.5 py-0.5 text-[var(--doc-ink-2)]">T맵 실도로</span>
                  : <span className="rounded bg-gray-200 px-1.5 py-0.5 text-[var(--doc-ink-2)]">직선 추정</span>}
                {v.tmapMinutes != null && <span className="text-[var(--doc-ink-2)]">약 {v.tmapMinutes}분{v.tmapKm != null ? ` · ${v.tmapKm}km` : ""}</span>}
              </div>
              <ol className="divide-y divide-[var(--doc-rule)]">
                <li className="flex items-center gap-2.5 bg-[var(--doc-grid-head)] px-3 py-2">
                  <span className="grid h-6 w-6 place-items-center rounded-[3px] border border-[var(--doc-rule-strong)] text-[11px] text-[var(--doc-ink-2)]">{isPickup ? "●" : "■"}</span>
                  <span className="text-[12px] font-bold text-[var(--doc-ink-2)]">{isPickup ? (sug.depot ? `${sug.depot.name.replace(/^차고지 · /, "차고지 ")} 출발` : "출발") : `${sug.academy.name} 출발`}</span>
                  <label className="ml-auto flex items-center gap-1 text-[11px] font-bold text-[var(--doc-ink-2)] print:hidden">
                    출발
                    <input type="time" value={v.departTime ?? ""} onChange={(e) => setDepartTime(vIdx, e.target.value)} className="rounded-[3px] border border-[var(--doc-rule)] px-1.5 py-0.5 text-[11.5px] font-bold text-[var(--doc-ink-2)]" />
                  </label>
                  {v.departTime && <span className="hidden text-[11.5px] font-bold text-[var(--doc-ink-2)] print:inline">{v.departTime} 출발</span>}
                </li>
                {v.stops.map((s, sIdx) => (
                  <li key={sIdx}
                    ...
                    className={`flex items-start gap-2 px-3 py-2.5 ${s.isHub ? "bg-[var(--doc-accent-soft)]/70 " : ""} ${drag && drag.v === vIdx && drag.s === sIdx ? "opacity-40" : ""} ${stuDrag && s.isHub ? " ring-inset " : ""}`}>
                    <span draggable onDragStart={(e) => { setDrag({ v: vIdx, s: sIdx }); e.dataTransfer.effectAllowed = "move"; }} onDragEnd={() => setDrag(null)} title="드래그해서 순서 변경" className="mt-0.5 shrink-0 cursor-move select-none text-base leading-none text-[var(--doc-ink-3)] hover:text-[var(--doc-ink-2)] print:hidden">⠿</span>
                    <span className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-[3px] text-[11px] font-bold text-white ${s.isHub ? "bg-green-600" : "bg-[var(--doc-accent)]"}`}>{s.isHub ? "🆓" : sIdx + 1}</span>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-[13px] font-bold text-[var(--doc-ink)]">
                        {s.label}
                        {s.isHub
                          ? <>
                              <span className="ml-1 rounded bg-green-600 px-1.5 text-[10px] font-bold text-white">무료 탑승 거점</span>
                              {s.students.length > 0 && <span className="ml-1 rounded bg-lime-200 px-1.5 text-[10px] font-bold text-[var(--doc-ink)]">{s.students.length}명</span>}
                            </>
                          : <span className="ml-1 rounded bg-lime-200 px-1.5 text-[10px] font-bold text-[var(--doc-ink)]">{s.students.length}명</span>}
                        {s.approx && !s.isHub && <span className="ml-1 rounded bg-[var(--doc-grid-head)] px-1.5 text-[10px] font-bold text-[var(--doc-warn)]">추정</span>}
                      </div>
                      <div className="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5 text-[11.5px] text-[var(--doc-ink-2)]">
                        {s.isHub ? (
                          <>
                            {s.students.map((st, i) => { const t = tel(st.parentPhone); const canOpen = rosterEditable && !!st.applicationId; return <span key={i} className="font-bold text-[var(--doc-accent)]"><button type="button" onClick={() => canOpen && setOpenStu({ applicationId: st.applicationId!, rosterId: st.rosterId, requestId: st.requestId })} className={canOpen ? "underline decoration-dotted underline-offset-2 hover:text-[var(--doc-accent)]" : ""}>{st.name}{st.grade ? `·${st.grade}` : ""}</button>{t && <a href={t} draggable={false} className="ml-0.5 text-[var(--doc-accent)]"><span className="material-symbols-outlined" style={{ fontSize: 14 }}>call</span></a>}</span>; })}
                            <span className="text-[var(--doc-accent)]">학생을 여기로 끌어다 놓으면 무료 거점 {isPickup ? "탑승" : "하차"}으로 지정됩니다 · 워크인 정원 별도</span>
                          </>
                        ) : (
                          s.students.map((st, i) => {
                            const t = tel(st.parentPhone);
                            const canOpen = rosterEditable && !!st.applicationId;
                            return <span key={i} draggable={rosterEditable} onDragStart={(e) => { if (!rosterEditable) return; setStuDrag({ v: vIdx, s: sIdx, i }); e.dataTransfer.effectAllowed = "move"; }} onDragEnd={() => setStuDrag(null)} title={rosterEditable ? "드래그해서 무료 거점으로 이동 · 이름 클릭 시 상세" : undefined} className={rosterEditable ? "cursor-grab select-none rounded px-0.5 hover:bg-[var(--doc-accent-soft)]" : "select-none rounded px-0.5"}>
                              <button type="button" draggable={false} onClick={() => canOpen && setOpenStu({ applicationId: st.applicationId!, rosterId: st.rosterId, requestId: st.requestId })} className={canOpen ? "font-bold underline decoration-dotted underline-offset-2 hover:text-[var(--doc-accent)]" : "font-bold"}>{st.name}{st.grade ? `·${st.grade}` : ""}</button>{t && <a href={t} draggable={false} className="ml-0.5 font-bold text-[var(--doc-accent)]"><span className="material-symbols-outlined" style={{ fontSize: 14 }}>call</span></a>}</span>;
                          })
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-0.5 print:hidden">
                      <div className="flex items-center gap-1">
                        <input type="time" value={stopEtaHHMM(s)} onChange={(e) => setStopEta(vIdx, sIdx, e.target.value)}
                          ...
                          className={`rounded-[3px] border px-1.5 py-0.5 text-[11.5px] font-bold ${s.etaManual != null ? "border-[var(--doc-rule)] text-[var(--doc-ink-2)] " : "border-[var(--doc-rule)] text-[var(--doc-ink-2)] "}`} />
                        <span className="whitespace-nowrap text-[10px] font-bold text-[var(--doc-ink-3)]">{isPickup ? "승차" : "하차"}</span>
                      </div>
                      {s.etaManual != null && (
                        <div className="flex items-center gap-1">
                          <span className="rounded bg-[var(--doc-grid-head)] px-1 text-[9px] font-bold text-[var(--doc-ink-2)]">확정(수정됨)</span>
                          <button type="button" onClick={() => resetStopEta(vIdx, sIdx)} title="자동 계산값으로 되돌립니다"
                            className="text-[10px] font-bold text-[var(--doc-ink-3)] hover:text-[var(--doc-ink-2)]">↺ 다시 계산</button>
                        </div>
                      )}
                    </div>
                    {s.etaLabel && <span className="hidden whitespace-nowrap text-[11.5px] font-bold text-black print:inline">{s.etaLabel}</span>}
                  </li>
                ))}
                <li className="flex items-center gap-2.5 bg-[var(--doc-grid-head)] px-3 py-2.5">
                  <span className="grid h-6 w-6 place-items-center rounded-[3px] border border-[var(--doc-ink)] text-[11px] text-[var(--doc-ink)]">{isPickup ? "■" : "●"}</span>
                  <span className="text-[12.5px] font-bold text-[var(--doc-ink-2)]">{isPickup ? `${sug.academy.name} 도착` : (sug.depot ? `${sug.depot.name.replace(/^차고지 · /, "차고지 ")} 복귀` : "복귀")}</span>
                  {isPickup && v.arriveTime && <span className="ml-auto text-[11.5px] font-bold text-[var(--doc-ink-2)]">{v.arriveTime} 도착</span>}
                  {!isPickup && v.depotTime && <span className="ml-auto text-[11.5px] font-bold text-[var(--doc-ink-2)]">{v.depotTime} 복귀</span>}
                </li>
              </ol>
            </div>
          ))}
        ...
        {sug.vehicles.length > 0 && (
          <aside className="mt-3 lg:mt-0 lg:sticky lg:top-4 print:hidden">
            <div className="overflow-hidden rounded-[6px] border border-[var(--doc-rule)]">
              <div className="flex items-center justify-between gap-2 px-3 py-2" style={{ background: "var(--doc-grid-head)", color: "var(--doc-ink)", borderBottom: "1.5px solid var(--doc-ink)" }}>
                <span className="text-xs font-bold">{isPickup ? "등원" : "하원"} 경로{rerouting ? " · 재계산 중" : ""}</span>
                {sug.vehicles.length > 1 && (
                  <select value={activeMapIdx} onChange={(e) => setMapVehicle(Number(e.target.value))} className="rounded-[3px] px-2 py-1 text-[11px] font-semibold" style={{ border: "1px solid var(--doc-rule-strong)", color: "var(--doc-ink)", background: "var(--doc-surface)" }}>
      ...
      {sug.unassigned.length > 0 && (
        <div className="mt-3 rounded-[3px] border border-[var(--doc-warn)] bg-[var(--doc-grid-head)] p-3">
          <div className="text-[12.5px] font-bold text-[var(--doc-warn)]">좌표가 없어 배차하지 못한 학생 {sug.unassigned.length}명</div>
          <div className="mt-1 text-[12px] text-[var(--doc-warn)]">{sug.unassigned.map((u) => `${u.name}(${u.label ?? "위치없음"})`).join(", ")}</div>
        </div>
      )}
