...
| 인스타그램 자동 누적 | 완료 | 매일 1회 cron으로 새 인스타 게시물만 `GalleryPost`에 추가 |

## 작업 로그
- 2026-07-07: 인스타그램 자동 가져오기 cron을 매시간에서 매일 1회(한국시간 03:10)로 조정.
...
## 다음에 할 것
- 배포 후 Vercel cron `/api/cron/instagram-gallery`가 매일 1회 실행되는지 확인.
- 운영자가 바로 반영하고 싶을 때는 관리자 갤러리의 `인스타 가져오기` 버튼을 수동으로 사용.
