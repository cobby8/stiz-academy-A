...
## 현재 작업
- 작업명: 관리자 갤러리 아이콘 폰트 요청 제거
- 상태: 빌드 검증 완료
- 범위: `/admin/gallery` 목록/업로드 모달 단순 아이콘
- 기준일: 2026-07-09
...
## 작업 로그
- 2026-07-09: `/admin/gallery` 목록과 업로드/수정 모달의 업로드/동기화/저장/공개상태/수정/삭제 아이콘을 `FontFreeIcon`으로 바꿔 관리자 갤러리 route의 Material Symbols 요청 흔적을 제거함.
...
## 구현 기록
- 변경 파일: `src/app/admin/gallery/GalleryAdminClient.tsx`, `src/app/admin/gallery/GalleryPostFormModal.tsx`, `src/components/ui/FontFreeIcon.tsx`
- 주요 변경: 관리자 갤러리 목록/업로드 모달의 단순 아이콘을 `FontFreeIcon`으로 전환.
- 적용 범위: `/admin/gallery`.

## 테스트 결과
- `npx.cmd tsc --noEmit` 통과
- `npx.cmd next build` 통과
- `src/app/admin/gallery`와 `.next/server/app/admin/gallery`에서 `material-symbols-outlined`, `fonts.googleapis`, `material-symbols-css` 0건 확인
- 빌드 중 Supabase DB 접속 실패 로그는 fallback 처리되어 빌드 종료 코드는 0.

## 다음에 할 것
- 다음 속도 개선 후보: `/admin/apply`의 남은 Material Symbols 사용처를 `FontFreeIcon` 또는 로컬 아이콘으로 전환.
