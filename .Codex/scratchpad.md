...
| 항목 | 상태 | 메모 |
| --- | --- | --- |
| 날짜 위치 | 완료 | 신청일/체험일을 상태 왼쪽으로 이동 |
| 기본 정렬 | 완료 | 체험일 오름차순, 미입력은 하단 정렬 |
| 날짜 필터 | 완료 | 전체/오늘 이후/오늘/이번 주/지난 일정/미입력 선택 필터 추가 |
| 검증 | 완료 | TypeScript, ESLint, 신청 관리 UX 테스트, diff check 통과 |

## 구현 기록
- `src/app/admin/trial/TrialCrmClient.tsx`: 체험신청 목록을 신청일/체험일 우선 컬럼으로 재배치하고 체험일 필터/오름차순 정렬 추가.
- `tests/application-management-ux.test.mjs`: 날짜 우선 컬럼 순서, 체험일 필터, 오름차순 정렬 회귀 테스트 갱신.
