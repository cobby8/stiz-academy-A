...
- 목표: 유니폼 추가주문을 자체 폼으로 접수하고 STIZ 본사 주문 API로 서버간 전송한다.
- 현재 단계: 자체 폼·DB 원장·본사 HMAC 전송·관리자 상태 화면 구현 및 로컬 검증 완료. 운영 DB 마이그레이션 적용과 실제 본사 전송 테스트는 별도 승인 필요.
...
| 유니폼 본사 연동 | developer + tester | 완료 | 자체 폼·DB 원장·HMAC 전송·관리자 상태 화면·로컬 빌드 검증 |
...
## 작업 로그 (최근 10건)
- 2026-08-31: 유니폼 자체 신청 폼, DB 주문 원장, STIZ 본사 HMAC 전송, 보호된 재시도 엔드포인트, 관리자 상태 화면을 구현했다. tsc·Prisma validate·유니폼 테스트 6건·빌드 통과, 운영 DB·외부 주문·문자·배포 없음.
