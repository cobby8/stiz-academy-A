...
- 주요 변경: 체험수업 CRM 페이지를 Suspense로 감싸 skeleton을 먼저 보여주고, `getTrialLeads()`/`getTrialStats()` 조회는 안쪽 서버 컴포넌트에서 스트리밍.
- 적용 범위: `/admin/trial`
...
## 다음에 할 것
- 다음 속도 개선 후보: `/admin/finance`, `/admin/gallery`, `/admin/notices`처럼 목록/통계/설정 조회를 첫 진입에 함께 가져오는 화면을 추가 점검.
