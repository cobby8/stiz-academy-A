        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">수업 사진과 영상을 업로드하고 학부모에게 공유하세요</p>
                </div>
                <button
                ...
                <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => resetForm()}>
                    <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">반 (선택)</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">제목 (선택)</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">사진/영상 업로드</label>
                                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 dark:bg-gray-900 transition">
                                    <Upload className="text-gray-400 mb-2" size={24} />
                                    <span className="text-sm text-gray-500 dark:text-gray-400">{uploading ? "업로드 중..." : "클릭하여 파일 선택"}</span>
                            ...
                            <div className="flex items-center gap-2">
                                <input type="checkbox" id="isPublic" checked={isPublic} onChange={e => setIsPublic(e.target.checked)}
                                    className="rounded" />
                                <label htmlFor="isPublic" className="text-sm text-gray-700 dark:text-gray-200">홈페이지 갤러리에 공개</label>
                            </div>
                        </div>
            ...
            {posts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {posts.map(post => {
                        ...
                        return (
                            <div key={post.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 transition-colors duration-300 shadow-sm overflow-hidden hover:shadow-md transition">
                                {/* Thumbnail */}
                                ...
                                <div className="p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <h3 className="font-bold text-gray-900 dark:text-white text-sm truncate">{post.title || "제목 없음"}</h3>
                                    ...
                                    <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
                                        <button onClick={() => startEdit(post)}
                                            className="flex items-center gap-1 text-xs text-gray-500 hover:text-brand-orange-500 dark:text-brand-neon-lime transition">
                                            <Edit2 size={14} /> 수정
                                        </button>
                                        <button onClick={() => handleDelete(post.id)}
                                            className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 hover:text-red-500 transition">
                                            <Trash2 size={14} /> 삭제
                                        </button>
                                    </div>
                                </div>
