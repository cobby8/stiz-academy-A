...
- 현재 단계: 격리 PostgreSQL37·브라우저 모의API 기능13·회귀88 통과. 별도 진단에서 미저장 새로고침 유실 재현(미수정). 운영 통합 E2E·시트 대체 미완료, 저장 기본 off. 카카오 예약 automation-2 해제 완료.
...
| 사이트 월 장부 2차 | PM + 모델 + UI + QA | 격리 UI 검증·보완1건 | DB37·브라우저 기능13·회귀88·tsc·lint 통과; 미저장 새로고침 보호 후속, 운영 DB/배포 미적용 |
...
## 확인보류
- 복수 반은 시트 별도 행. 미저장 이탈 보호·운영 통합 E2E·스키마 호환·이관대조·할인/이월 잔액 자동검증 필요. 시트 중단·운영 쓰기·배포 미승인.
