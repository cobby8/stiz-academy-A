...
## 현재 작업
- 작업명: 홈페이지 갤러리 게시와 인스타그램 게시 상태 분리
- 상태: 검증 완료
- 범위: 선생님 빠른 업로드, 관리자 갤러리 초안 게시, 소셜 초안 상태
- 기준일: 2026-07-08
...
## 구현 기록
- 변경 파일: `src/lib/socialDrafts.ts`, `src/app/actions/social-posts.ts`, `src/app/staff/quick-post/QuickPostClient.tsx`, `src/app/admin/gallery/GalleryAdminClient.tsx`
- 주요 변경: 초안을 홈페이지 갤러리에 먼저 반영한 뒤 인스타그램 게시를 이어서 실행하고, 중간에 멈긴 `PUBLISHING` 상태는 재시도 가능하게 표시.
- 적용 범위: 선생님 바로 게시, 관리자 선생님 업로드 대기 목록.
