...
- 현재 단계: 점검표에 이어 사이트 월 운영 장부의 저장/확정/재열기·감면 근거·버전 감사를 구현. 운영 DB 적용·실제 E2E·시트 대체는 미완료, 저장 기능 기본 off.
...
| 사이트 월 장부 2차 | PM + 모델 + UI + QA | 로컬 검증 완료 | 회귀88·tsc·Prisma·lint·검토 통과; 초안/확정/재열기·감사 거래, 운영 DB/배포 미적용 |
...
## 확인보류
- 복수 반은 시트 별도 행. 사이트 월 장부 코드 이후 테스트 DB·브라우저 E2E·이관대조·할인/이월 잔액 근거 자동검증이 필요. 시트 중단·운영 쓰기·배포 미승인.
