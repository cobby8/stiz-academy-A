...
test("공개 신청은 저장 성공 뒤 접수 안내를 보내고 같은 요청은 미완료 알림을 복구한다", () => {
  assert.match(service, /return duplicateApplicationResponse\(duplicate, season\.title\)/);
  ...
  assert.match(service, /latest\.status === "PENDING"/);
  assert.match(service, /latest\.status === "FAILED" && latest\.errorCode !== "FAILED_DELIVERY_UNCERTAIN"/);
  assert.match(service, /WHERE id = \$1 AND status = 'FAILED'/);
  assert.match(service, /COALESCE\("errorCode", ''\) <> 'FAILED_DELIVERY_UNCERTAIN'/);
});
...
test("공개 신청 폼은 회원 구분과 회원별 가격을 같은 payload에 사용한다", () => {
  ...
  assert.match(apply, /type ApplicantType = "NEW" \| "EXISTING"/);
  assert.match(apply, /name="applicantType"/);
  assert.match(apply, /applicantType,/);
  assert.match(apply, /item\.newApplicantPrice \?\? item\.price/);
  assert.match(apply, /item\.existingApplicantPrice \?\? item\.price/);
  assert.match(apply, /formatWon\(applicantPrice\(item, applicantType\)\)/);
  assert.match(service, /newApplicantPrice: offering\.newApplicantPrice/);
  assert.match(service, /existingApplicantPrice: offering\.existingApplicantPrice/);
  assert.match(types, /newApplicantPrice\?: number \| null/);
});
