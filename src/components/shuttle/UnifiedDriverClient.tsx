  ...
  return (
    ...
    <div data-density="driver" className="mx-auto max-w-lg px-3 pb-28"
      ...
      <header className="sticky top-0 z-10 -mx-3 mb-3 border-b border-[var(--doc-rule)] bg-[var(--doc-surface)] px-4 py-4">
        ...
        <p className="mt-0.5 text-[15px] font-bold text-[var(--doc-ink-2)]">
        ...
        <div className="mb-2 flex items-center justify-end gap-2">
          {!editing ? (
            <button type="button" onClick={() => setEditing(true)}
              className="rounded-[3px] border-2 border-[var(--doc-rule)] px-3 py-1.5 text-[13px] font-bold text-[var(--doc-ink-2)] active:bg-[var(--doc-grid-head)]">
              ↕ 순서 편집
            </button>
          ) : (
            <>
              ...
              <button type="button" onClick={() => { setEditing(false); setSortOverride({}); }}
                className="rounded-[3px] border-2 border-[var(--doc-rule)] px-3 py-1.5 text-[13px] font-bold text-[var(--doc-ink-2)]">취소</button>
            </>
          )}
        </div>
      )}
      {editing && <p className="mb-2 rounded-[3px] bg-[var(--doc-grid-head)] px-3 py-2 text-[13px] font-bold text-[var(--doc-ink-2)]">↕ 버튼으로 순서를 바꾼 뒤 &quot;순서 고정 요청&quot;을 눌러주세요</p>}

      {stopRows.length === 0 && (
        <p className="rounded-[6px] bg-[var(--doc-grid-head)] px-4 py-6 text-center text-[16px] font-bold text-[var(--doc-ink-3)]">오늘은 운행이 없습니다.</p>
      )}
      ...
      <ol className="space-y-2.5">
        {ordered.map((row) => {
          ...
          if (row.isTerminal) {
            return (
              <li key={row.key} className="flex items-center gap-2 rounded-[3px] bg-[var(--doc-grid-head)] px-4 py-2.5 text-[15px] font-bold text-[var(--doc-ink-2)]">
                <span className="shrink-0 text-[15px] font-bold text-[var(--doc-ink-2)]">{row.time}</span>
                <span className="min-w-0 flex-1">{row.label}</span>
                {row.subLabel && <span className="shrink-0 text-[13px] font-semibold text-[var(--doc-ink-3)]">{row.subLabel}</span>}
              </li>
            );
          }
          seq += 1;
          ...
          return (
            <li key={row.key} className={`rounded-[6px] border-2 p-3.5 ${
 editing ? "border-[var(--doc-rule)] bg-[var(--doc-grid-head)]" : row.warn ? "border-[var(--doc-warn)] bg-[var(--doc-grid-head)]" : row.isHub ? "border-[var(--doc-accent)] bg-[var(--doc-accent-soft)]" : "border-[var(--doc-rule)] bg-[var(--doc-surface)]"
 }`}>
              <div className="flex items-center gap-2.5">
                {editing ? (
                  <div className="flex shrink-0 flex-col gap-0.5">
                    <button type="button" onClick={() => moveRow(row, -1)}
                      className="h-7 w-7 rounded-[3px] border border-[var(--doc-rule)] text-[16px] font-bold text-[var(--doc-ink-2)]">▲</button>
                    <button type="button" onClick={() => moveRow(row, 1)}
                      className="h-7 w-7 rounded-[3px] border border-[var(--doc-rule)] text-[16px] font-bold text-[var(--doc-ink-2)]">▼</button>
                  </div>
                ) : (
                  <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-[3px] text-[15px] font-bold text-white ${row.isHub ? "bg-green-600" : "bg-brand-orange-500"}`}>
                    {row.isHub ? "🆓" : seq}
                  </span>
                )}
                <span className="min-w-0 flex-1 text-[18px] font-bold leading-tight text-[var(--doc-ink)]">{row.label}</span>
                <span className={`shrink-0 text-[16px] font-bold ${row.time ? "text-[var(--doc-ink-2)]" : "text-[var(--doc-ink-3)]"}`}>{row.time ?? "시간 미정"}</span>
              </div>

              {/* 종류·방향 배지 + 노선 이름 */}
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                <span className={`rounded-[3px] px-2 py-0.5 text-[13px] font-bold text-white ${row.kind === "SEASONAL" ? "bg-indigo-600" : "bg-teal-600"}`}>
                  {row.kind === "SEASONAL" ? "특강" : "정규"}
                </span>
                <span className={`rounded-[3px] px-2 py-0.5 text-[13px] font-bold text-white ${isPickup ? "bg-blue-600" : "bg-orange-600"}`}>
                  {isPickup ? "⬆ 등원" : "⬇ 하원"}
                </span>
                {row.groupLabel && <span className="text-[13px] font-bold text-[var(--doc-ink-2)]">{row.groupLabel}</span>}
                {/* 원장이 아직 노선을 확정하지 않은 요일 — 시트 명단 순서라는 걸 기사님께 알린다 */}
                {row.pending && <span className="rounded-[3px] bg-[var(--doc-grid-head)] px-1.5 py-0.5 text-[12px] font-bold text-[var(--doc-warn)]">임시 순서 · 확정 전</span>}
                {!editing && (
                  <button type="button" onClick={() => setReqModal({ targetName: row.label, defaultType: "LOCATION" })}
                    className="ml-auto rounded-[3px] border border-[var(--doc-rule)] px-2 py-1 text-[12px] font-bold text-[var(--doc-ink-2)] active:bg-[var(--doc-grid-head)]">
                    요청
                  </button>
                )}
              </div>
              ...
              {!editing && url && (
                <a href={url} className="mt-2 flex h-12 items-center justify-center gap-1.5 rounded-[3px] bg-blue-600 text-[16px] font-bold text-white active:bg-blue-700">🧭 T맵 길안내</a>
              )}
              {row.isHub && row.riders.length === 0 && <p className="mt-1.5 text-[15px] font-bold text-[var(--doc-accent)]">무료 거점(워크인, 정원 별도)</p>}
              ...
              {row.riders.length > 0 && (
                <div className="mt-2.5 space-y-2.5">
                  {row.riders.map((rider) => {
                    ...
                    return (
                      <div key={rowKey} className={`rounded-[3px] p-2.5 ${lockedAbsent ? "bg-[var(--doc-crit-soft)]" : "bg-[var(--doc-grid-head)]"}`}>
                        <div className="flex items-center gap-2">
                          <div className="min-w-0 flex-1">
                            <div className="flex flex-wrap items-baseline gap-1.5">
                              <span className={`text-[19px] font-bold ${lockedAbsent ? "text-[var(--doc-ink-3)] line-through" : "text-[var(--doc-ink)]"}`}>{rider.name}</span>
                              {rider.grade && <span className="text-[14px] text-[var(--doc-ink-2)]">{rider.grade}</span>}
                              {lockedAbsent && <span className="rounded-[3px] bg-red-500 px-2 py-0.5 text-[13px] font-bold text-white">오늘 결석</span>}
                              {rider.absent && !lockedAbsent && status !== "BOARDED" && status !== "SELF" && (
                                <span className="rounded-[3px] bg-amber-400 px-2 py-0.5 text-[13px] font-bold text-white">결석예정</span>
                              )}
                              {!lockedAbsent && status === "NOSHOW" && <span className="rounded-[3px] bg-red-500 px-2 py-0.5 text-[13px] font-bold text-white">결석</span>}
                              {!lockedAbsent && status === "SELF" && <span className="rounded-[3px] bg-violet-600 px-2 py-0.5 text-[13px] font-bold text-white">{selfLabel}</span>}
                              {/* "오늘만" 셔틀 변경. 결석과 다른 색으로 둔다 — 아이는 수업에 오고,
                                  기사님이 태우지 않거나 다른 곳으로 가야 한다는 뜻이다. */}
                              {rider.shuttleNote && <span className="rounded-[3px] bg-blue-600 px-2 py-0.5 text-[13px] font-bold text-white">{rider.shuttleNote}</span>}
                            </div>
                          ...
                          {lockedAbsent ? (
                            ...
                            <>
                              ...
                              <button type="button" disabled={isBusy} onClick={() => { setMenuKey(null); setStatus(rider, "BOARDED"); }}
                                className={`h-14 min-w-[68px] rounded-[3px] text-[16px] font-bold ${status === "BOARDED" ? "bg-green-600 text-white" : "border-2 border-[var(--doc-accent)] text-[var(--doc-accent)]"}`}>
                                {isPickup ? "탑승" : "하차"}
                              </button>
                              <button type="button" onClick={() => setMenuKey(menuKey === rowKey ? null : rowKey)}
                                className={`h-14 min-w-[68px] rounded-[3px] text-[16px] font-bold ${status === "NOSHOW" || status === "SELF" ? "bg-gray-700 text-white" : "border-2 border-[var(--doc-rule)] text-[var(--doc-ink-2)]"}`}>
                                미{isPickup ? "탑승" : "하차"}
                              </button>
                            </>
                          )}
