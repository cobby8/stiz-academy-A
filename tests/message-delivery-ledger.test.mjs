...
test("전화번호는 비밀키 HMAC, 본문은 SHA-256, 중복키는 안정적인 입력으로 만든다", () => {
  assert.match(helper, /MESSAGE_PRIVACY_HMAC_SECRET/);
  assert.match(helper, /Buffer\.byteLength\(value, "utf8"\) >= 32/);
  assert.match(helper, /createHmac\("sha256", privacySecret\(\)\)/);
  assert.match(helper, /createHash\("sha256"\)/);
  assert.match(helper, /MESSAGE_PRIVACY_HMAC_SECRET_INVALID/);
