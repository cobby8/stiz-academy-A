...
## 현재 작업
- 작업명: 선생님/관리자 갤러리 업로드 UX 1차 개선
- 상태: 검증 완료
- 범위: 선생님 빠른 업로드, 관리자 갤러리 수동 업로드, 공통 이미지 업로드 도우미
- 기준일: 2026-07-08
...
## 구현 기록
- 변경 파일: `src/lib/clientImageUpload.ts`, `src/app/staff/quick-post/QuickPostClient.tsx`, `src/app/admin/gallery/GalleryAdminClient.tsx`
- 주요 변경: 공통 이미지 압축/업로드 도우미를 만들고, 여러 사진을 3장씩 병렬 업로드하며 진행률과 성공 안내를 표시.
- 적용 범위: 선생님 사진 올리기, 관리자 갤러리 새 게시물/수정 모달.
...
## 다음에 할 것
- 인스타 게시를 백그라운드 상태 처리로 분리해 홈페이지 게시 성공을 더 빨리 보여주는 2차 개선.
