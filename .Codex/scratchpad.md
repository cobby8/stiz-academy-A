...
## 현재 작업
- 작업명: 원생 상세 활동 백그라운드 로딩
- 상태: 빌드 검증 완료
- 범위: `/admin/students/[id]`, `/api/admin/students/[id]/activity`
- 기준일: 2026-07-10
...
| 원생 상세 활동 백그라운드 로딩 | 완료 | 원생 상세의 활동/출석/결제/갤러리 조회를 클라이언트 API로 지연 로딩 |
...
## 구현 기록
- 변경 파일: `src/app/admin/students/[id]/page.tsx`, `src/app/admin/students/[id]/StudentDetailClient.tsx`, `src/app/api/admin/students/[id]/activity/route.ts`
- 주요 변경: 원생 상세의 무거운 활동 데이터 조회를 클라이언트 API로 분리하고, 상세 skeleton/재시도 UI를 클라이언트에서 표시.
- 적용 범위: `/admin/students/[id]`
...
## 다음에 할 것
- 다음 속도 개선 후보: 반 상세처럼 상세 데이터가 무거운 화면을 추가 점검.
