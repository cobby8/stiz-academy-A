...
- 목표: 카카오 채널 학부모를 최초 1회 인증하고 이후 자녀를 자동 인식해 자연어 요청을 접수한다.
- 현재 단계: 인증 연결·21종 분류·확인 접수·관리자 접수함 구현 및 로컬 검증 완료. DB 적용·배포·카카오 연결 승인 대기.
- 운영 반영: 운영 DB 변경, 배포, 외부 알림 없음.
- 별도 실행 대기: 카카오 챗봇 관리자센터 권한·봇/채널 연결·환경 비밀키 설정 확인.
