...
- **기존 기능 무조건 보존**: 디자인 개편이나 리팩토링 시에도 기존의 프로젝트 기능(로직)은 모두 보존한다.
- **UX 흐름 동일 유지**: 기존 사용자가 겪던 UX 단계 및 흐름은 동일하게 유지해야 한다.
- **변경 전 동의 필수**: 구조적, 시각적 변경이 필요한 부분은 반드시 사전 기획을 통해 사용자에게 동의를 구한 후 작업을 진행한다.
