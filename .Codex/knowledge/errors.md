...
## 카카오 상담 전환 뒤 후속 처리 단절과 동일 상태 경합 (2026-09-09)
- 원인: CONSULTATION이 검토 가능 상태에서 빠져 있었고 UPDATE는 허용 상태 집합만 비교해 같은 상태의 오래된 화면도 다시 저장할 수 있었다.
- 수정: 상담에서 추가확인·검증 이관·상담종결로 진행한다. 상담종결은 수강·청구 반영이나 학부모 발송과 분리하며 사유/처리자/시각과 감사 저장을 같은 거래로 묶는다. 목록에서 받은 updatedAt 원문을 상태와 함께 UPDATE 조건으로 비교한다.
- 예방: 마이크로초가 손실되는 JS Date 버전 변환을 피한다. 편집 시작 버전을 고정하고 충돌 시 최신 목록 재조회를 요구한다. 가짜 거래의 경쟁·감사 실패 롤백 행동 검증은 실제 DB 동시성/브라우저 E2E 검증과 구분한다.

## 예약 설정 백업이 조회 실패를 성공으로 숨김 (2026-09-09)
- 원인: cron/backup의 safeQuery가 예외를 빈 배열로 바꿔 불완전한 설정 백업 업로드와 과거 파일 정리를 계속했다.
- 수정: 필수 7개 조회 실패는 저장 전 중단한다. Storage 단계 오류를 검사하고 저장 후 정리 실패는 backupSaved/filename/stage로 구분한다. 업로드 응답 유실/오류는 backupSaved:null과 후보 filename을 보존해 재조회를 요구한다. deleted는 삭제 응답에서 확인된 이름만 기록한다(삭제 응답 유실은 실제 미삭제를 뜻하지 않음). 공급자 예외 원문은 출력하지 않는다.
- 범위: 설정 백업만 해당. 학생·청구·월 장부 전체 백업, 수동 admin/backup 및 admin/backup-now의 동일 결함은 별도 후속이다. 조회의 한 시점 일치·복원 검증·목록 200건 초과 정리·자동 재시도 중복 방지는 이 그룹이 보장하지 않는다.
- 예방: 실제 라우트에 가짜 DB/Storage를 연결한 tests/cron-backup.test.mjs로 정상 빈 자료와 조회·업로드·정리 실패를 구별한다. 실제 운영 Storage 호출 없이 검사한다.

## 통합 테스트 loopback Origin 불일치 (2026-09-04)
- 실제 NextRequest는 URL의 `127.0.0.1`을 `localhost`로 정규화하지만 Origin 헤더는 바꾸지 않는다. 테스트가 숫자 주소로 POST하면 정상 입력도 403이 되어 입력 오류 400 검증 전에 중단된다.
- 테스트 브라우저·직접 API 요청 URL을 `localhost`로 통일한다. 정적 서버 바인딩과 DB 접속은 `127.0.0.1`로 유지하며 실제 Origin 검사나 헤더를 우회하지 않는다. 실패한 임시 DB도 정상 종료했다.

## 통합 테스트의 모듈별 VM 객체 영역 불일치 (2026-09-04)
- 운영 모델은 일반 객체만 허용하도록 객체 원형을 검사한다. 테스트에서 API·모델을 각각 별도 VM 영역에 넣으면 정상 JSON이나 PostgreSQL JSON 응답도 서로 다른 객체 원형으로 인식해 정상 저장을 400으로 거절한다.
- 운영 검증을 완화하지 않고, 신뢰하는 저장소 코드만 같은 객체 영역에 컴파일하는 테스트 로더로 연결한다. 의존 모듈 허용 목록과 요청별 process/env 대체는 유지한다. 이 로더는 비신뢰 코드를 실행하는 보안 샌드박스로 사용하지 않는다.
- 최초 저장 응답 유실 뒤 화면은 아직 dirty이므로 재조회 confirm을 명시적으로 확인·승인해야 한다. Playwright 기본 dismiss에 맡기면 GET이 실행되지 않는다.

## pg_restore의 기본 ACL 표기 차이 (2026-09-04)
- 명시적 소유자 전용 권한은 복원 후 `relacl = NULL`(기본 소유자 권한)로 표현될 수 있다. 실제 접근 권한은 같아도 원문 문자열 비교는 실패한다.
- 보안 스냅샷은 `COALESCE(relacl, acldefault('r', relowner))`로 기본 권한을 펼쳐 비교한다. RLS·전체 데이터·명시적 외부 역할 권한은 계속 함께 검사하며 공개 권한 증가를 무시하지 않는다.

## 월 장부 미저장 새로고침 유실 — 로컬 수정·검증 완료 (2026-09-04)
- 격리된 실제 React 화면에서 기본 수강료를 100000→123456으로 편집한 뒤 새로고침하면 경고 없이 저장본 100000으로 돌아온다. 서버 자료가 지워진 것이 아니라 미저장 편집이 사라지는 문제다.
- 사용자 승인 후 beforeunload 경고와 문서 캡처 단계의 같은 창 링크 확인을 추가했다. POST 시작부터 재조회 성공까지 needsRefresh를 유지해 저장 성공·조회 실패 구간도 보호한다. 저장/결과 조회 중 링크는 차단하며 새로고침 취소 시 편집을 유지한다.
- 격리 브라우저19·회귀90·TypeScript·ESLint 통과. 링크 승인 확인창 1회와 저장 실패/결과 불일치 경고도 확인했다. 실제 Next 라우터/뒤로가기·강제 종료·모바일 전원 차단을 보장하지 않는다.
- 브라우저 테스트에서 label 안 select/textarea의 자식 내용이 접근 가능한 이름에 포함될 수 있다. 정확한 전체 문자열보다 고유 역할+라벨 접두사를 사용해 입력값·선택지 변화에도 대상을 찾도록 했다. 기능13·별도진단1을 두 번 완료했다.

## 임시 PostgreSQL 검사 환경·주소 표기 (2026-09-04)
- Windows 제한 토큰에서 `pg_ctl`이 `could not create restricted token: error code 87`로 실행 실패했다. 승인된 새 로컬 테스트 클러스터 실행만 권한 확장해 재시도했으며 기존 서버는 사용하지 않았다.
- `inet_server_addr()::text`는 `127.0.0.1/32`로 반환될 수 있다. 주소 검증은 `host(inet_server_addr())`로 정규화하고 포트·DB명·data_directory도 함께 대조한다. 첫 오류는 쓰기 전 차단되었다.
- 임시 서버는 Node 종료와 별개로 남을 수 있어 시작 Promise와 중단 신호를 조율한다. 소유 경로만 종료하고 pool 종료 대기는 제한하며, 실패 시 동일 가상 DB에 한정해 재종료한다. 실제 DB 37건 통과 후 종료를 확인했다.

## 월 장부 유효 입력과 요청 크기 제한 불일치 (2026-09-04)
- 20개 반·각 500자 한글 근거는 유효한 입력이지만 JSON 본문이 38KB를 넘어 기존 32KiB 상한에서 거부됐다. 128KiB로 맞추고 Content-Length와 관계없이 실제 수신 바이트를 제한한다.
- 최대 한글 입력 통과와 다중 chunk/허위 길이 헤더의 초과 요청 거부를 실행 테스트했다. 운영 DB를 호출하지 않는다.
- UI의 미선택 상태 빈 문자열은 검증 후 명시적으로 좁힌 타입을 사용해 TypeScript 오류를 해소했다.

## 월 청구 다반 누락 및 선택 발행 범위 확대 (2026-09-04)
- 템플릿별 HAVING COUNT(classId)=1은 다반 학생을 미리보기에서 숨긴다. 전체 활성 반 수를 먼저 집계하고 다반은 REVIEW로 표시·생성 제외한다. 프로그램이 달라도 예외가 아니다.
- 생성 후보를 제한해도 이후 ensureInvoicesForMonth가 월 전체 Payment를 갱신하면 제외 대상이 발행될 수 있다. 이번 생성의 RETURNING id만 3번째 인수로 전달하고 생성·갱신·집계 모든 쿼리에 동일 ID 조건을 붙였다. 빈 목록은 no-op. 모의 실행/소스 계약 회귀로 검증했다.
- 옛 HAVING을 기대하던 회귀는 REVIEW와 CREATE 한정이라는 의미 보존 계약으로 갱신했다. 신규 페이지 날짜는 공용 KST 헬퍼를 사용하고, 테스트의 module 지역변수는 Next lint와 충돌하므로 testModule로 쓴다.
- 범위를 새 Payment ID로 줄인 뒤 별도 거래를 유지하면 청구서 실패 시 Payment만 남아 재시도에서 SKIP된다. 생성과 청구서 연결을 같은 거래로 묶고 실패 롤백/재시도 흐름을 가짜 거래 기반 테스트로 검증했다. 실제 DB 경합/롤백은 미검증이다.

## 시트 월 합계가 반별 납부기록을 덮어쓰는 위험 (2026-09-04)
- 학생별 최신 Payment 하나를 고르면 반별 납부에 월 합계가 덮어써진다. 전체 월 기록을 합산하고 복수 반/기록은 REVIEW로 분리한다.
- 안전한 반별 증빙 연결 전까지 sheet-reconcile POST는 409로 차단한다. 청구서 생성과 전역 연체 갱신도 실행하지 않는다.
- 저장 배치에 대상 월이 없으면 빈 결과를 정상 일치로 표시하지 않는다. 실시간 시트와 저장 배치는 다르다.

## 대기 원장 선두 200건으로 후속 요청이 조회되지 않는 위험 (2026-09-04)
- appliedAt을 삼중 검증까지 비워둘 때 이미 이관한 요청이 LIMIT을 계속 차지할 수 있다.
- 조회에서 운영 멱등키가 존재하는 요청을 NOT EXISTS로 제외하고 트랜잭션 내부에서도 중복 검증한다.
- 수강변경 새 안전 정책에 맞춰 직접 사이트 UPDATE/자동 알림을 기대하던 테스트를 대기 원장/발송 HELD 검사로 바꿨다. 확대 회귀 148건 통과.
...
# 2026-08-26: 차량 변동 문자 최종 승인 없이 발송

- 현상: 차량 시간 변동 문자 11건을 수신자·변동값·문구 미리보기와 별도 최종 승인 없이 발송했다.
- 원인: 최초 요청의 `변동 알림 문자 보내야 해`를 준비 지시가 아니라 실행 승인으로 잘못 해석했고, 기존 스킬의 `action-time confirmation`도 구체적인 정지 조건이 없었다.
- 해결: 알림 준비와 발송을 분리한다. 수신자, 연락처, 이전/변경 값, 정확한 문구, 총건수를 미리 보여준 뒤 멈추고 그 미리보기에 대한 명시적 승인을 받아야만 발송한다.
- 예방: 초기 업무 요청, 묶음 승인, 과거 승인은 발송 승인이 아니다. 승인 후 대상·금액·시간·문구·건수가 하나라도 달라지면 승인 무효로 보고 다시 미리보기한다. 승인 전 감사 상태는 `AWAITING_NOTIFICATION_APPROVAL`로 `HELD` 처리한다.

# 2026-08-27: 정규 차량 시트 적용 월 상태 지연

- 현상: 8월 시트를 선택했지만 화면 상태가 갱신되기 전에 가져오기를 눌러 9월 원장 228건이 8월 내용으로 교체됐다.
- 원인: 월 입력값과 실제 제출 상태가 같은 값인지 행동 직전에 재확인하지 않았다.
- 해결: 잘못 저장된 228건을 8월로 이동한 뒤 실제 `9월차량` 탭 228건을 9월에 다시 저장하고 월별 건수를 검증했다.
- 예방: 가져오기 직전에 `적용 월`과 `확인 월`을 둘 다 읽어 목표 월과 일치할 때만 제출한다. 서버 응답에도 적용 월을 표시하고, 월별 기존 건수 교체는 대상 월을 다시 확인한다.

# 2026-08-27: 전체 ESLint 실행 시 Node 4GB 메모리 초과

- 현상: `npm run lint`가 약 114초 뒤 `Ineffective mark-compacts near heap limit`로 종료됐다.
...
## 2026-09-09 G4: 수동 설정 백업 실패·불확실 결과 처리
- 다운로드 GET의 필수 조회 실패는 500 오류이며 정상 첨부를 반환하지 않는다. 수동 저장 POST는 G1처럼 수집 성공 후 Storage에 접근하고 저장 불확실(null/후보파일)과 저장 후 정리 실패(true)를 구분한다.
- UI는 HTTP 성공과 success:true만 완료로 표시하며 불확실/정리실패/응답유실 안내와 진행중 중복 클릭 차단을 제공한다. 학생·출결·청구·월 장부는 범위 밖이다.
- 수동30·UI7·예약18·권한2=57 행동/회귀 및 tsc 통과, 독립 검토 통과. 실제 DB/Storage·브라우저 검증 아님. 복원 POST는 변경/실행하지 않았다. 다운로드 실패는 API 오류 페이지로 표시되며 별도 복구 화면은 없다.
- lint 잔여: 기존 복원 POST any1 오류, UI 다른기능3 경고. HEAD 비교로 기존 문제임을 확인했으며 이번 변경의 신규 lint 오류는 없다. 보존기간30일·목록200건 한계·기존 JSON형식 유지.
