...
- 기준일: 2026-07-12
- 문서 수: 5
- 최근 지식: 시간표는 구글시트 캐시를 계속 읽는 중간 단계를 거치되, `ScheduleSlot`/`ScheduleImportBatch`/`ScheduleImportIssue`로 DB 원본 전환 기반을 만들었다. 관리자 화면에서 이관 미리보기로 오류를 먼저 확인하고, 오류가 없을 때만 새 DB 원본에 반영한다. 관리자 화면에서는 대시보드는 핵심 요약을 먼저 보여주고 긴 목록은 최초 일부만 렌더링한다. 원생 전체 목록처럼 필요할 때만 보는 데이터는 자동 백그라운드 조회하지 않는다. 반복 선택 목록/설정/수납/통계/콘텐츠성 API와 주요 운영 데이터 API는 짧게 캐시하고, 저장 액션에서 태그를 무효화한다. 사용자 화면에서는 DB 인덱스/DDL/배경 카운트 자동 조회를 피하고, 인덱스 적용은 Prisma schema/SQL 운영 작업으로 분리한다.
