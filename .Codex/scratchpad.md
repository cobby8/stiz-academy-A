...
- 현재 단계: 월 운영 장부의 격리 PostgreSQL 실제 검사 37개·회귀 88개 통과. 운영 DB 적용·브라우저 E2E·시트 대체는 미완료, 저장 기능 기본 off. 카카오 문의 예약 automation-2 해제 완료.
...
| 사이트 월 장부 2차 | PM + 모델 + UI + QA | 로컬 DB 검증 완료 | 실제 PostgreSQL37·회귀88·tsc·lint 통과; 백업복원·권한·경합·롤백 확인, 운영 DB/배포 미적용 |
...
## 확인보류
- 복수 반은 시트 별도 행. 로컬 DB 검증 이후 브라우저 E2E·운영 스키마 호환·이관대조·할인/이월 잔액 자동검증 필요. 시트 중단·운영 쓰기·배포 미승인.
