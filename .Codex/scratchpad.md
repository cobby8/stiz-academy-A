...
- 현재 단계: 신규 승인 반 검증·카카오 신규 입구·후속 대기 표시·입학 체크리스트·수강 변경 안전 보류 구현, 회귀 148건 및 TypeScript 통과. 완전 자동화/운영 배포 미완료.
...
## 확인보류
- 다음 개발: 추가확인 실제 전달 승인 흐름, 수강변경 외부 동기화 후 원 신청 appliedAt 확정, 차액 PaymentInvoice 연결, 입학 완료 증거의 신청·반·월 직접 연결 및 영구 저장. 현재 수강변경은 사이트 직접반영 대신 HELD 원장만 생성하므로 운영 배포 전 후속 경로 검증 필요.
