...
| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| 토큰 해시 저장·만료·취소 | ✅ 통과 | 32바이트 난수, SHA-256, 원문 토큰 DB 미저장 |
| 학생 소유권·노출 방지 | ✅ 통과 | 링크의 학생 ID만 명령에 결합, URL 식별값 없음, 화면 이름 마스킹 |
| 공개 제출 안전성 | ✅ 통과 | DRAFT만 생성, 청구·알림 HELD, 외부 반영 함수 호출 없음 |
| 관리자 권한 | ✅ 통과 | 링크 생성·취소·요청 승인 모두 `requireAdmin()` |
| 멱등성·상태 UI | ✅ 통과 | 링크 범위 고유키, ACTIVE/EXPIRED/INVALID 분기 확인 |
| 1회용 링크 동시 제출 | ✅ 통과 | 조건부 UPDATE 영향행 1건 선점 후 같은 트랜잭션에서만 DRAFT 생성 |
| 활성 링크 조회·취소 UI | ✅ 통과 | 관리자 인증, 유효 링크 필터, 취소 액션과 목록 제거 확인 |
| 홈페이지 동시성 가드 | ✅ 통과 | 승인 당시 상태 조건 UPDATE, 영향행 1건 확인, 충돌 시 트랜잭션 롤백·HELD |
...
### 테스트 결과 (tester) — 2026-08-27 `Sun-8` 대표팀 셔틀비 전액 면제

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| 활성 `Sun-8` + 다른 정규반 동시 수강 | ✅ 통과 | 수강료는 유지, 학생의 월 셔틀비만 전액 0원 |
| 휴원 `Sun-8` 면제 근거 제외 | ✅ 통과 | 다른 활성 반의 셔틀비 정상 유지 |
| 퇴원·이월 `Sun-8` 면제 근거 제외 | ✅ 통과 | 추가 경계 테스트 2건 |
| 다른 일요일 반·유사 키 비면제 | ✅ 통과 | `Sun-7`, `Sun-08`은 셔틀비 유지 |
| CSV 임포트 슬롯 전달 | ✅ 통과 | 청구 계산 전 `extractSlotKeys`로 행별 슬롯 생성·전달 |
| 기존 청구 회귀 | ✅ 통과 | `studentBilling.test.ts` 27건 전체 통과 |
