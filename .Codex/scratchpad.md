...
- 목표: 정규 셔틀 정류장 좌표를 운영자가 안전하게 설정하고 이후 최적경로 계산에 사용할 수 있게 한다.
- 현재 단계: 정류장별 검색 후보 선택·지도 핀 조정·개별 저장 UX와 배차 누락 안내 간소화 구현 및 로컬 검증 완료.
...
- 정규 셔틀 좌표 설정: 정류장 한 곳씩 카카오 검색 후보를 선택하고 지도 핀을 조정한 뒤 저장한다. 배차 화면의 긴 누락 명단은 기본 접고 좌표 설정 이동 버튼을 제공한다.
...
## 작업 로그 (최근 10건)
- 2026-09-02: 정규 셔틀 좌표 설정을 정류장별 검색 후보·지도 핀 조정·개별 저장 방식으로 재구성하고 배차 누락 명단을 접었다. 셔틀 회귀 7건·eslint·tsc·diff-check 통과, 운영 좌표 저장·배포 없음.
