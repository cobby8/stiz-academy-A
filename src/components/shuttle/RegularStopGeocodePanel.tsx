...
type KakaoMarker = { getPosition: () => KakaoLatLng };
type KakaoSdk = { maps: {
  load: (cb: () => void) => void;
  ...
  event: { addListener: (target: KakaoMarker, event: string, cb: () => void) => void };
...
export default function RegularStopGeocodePanel({ stopNames, totalStopCount, initialCompletedCount }: { stopNames: string[]; totalStopCount: number; initialCompletedCount: number }) {
  ...
  useEffect(() => {
    if (!chosen || !sdk || !mapElement.current) return;
    const center = new sdk.maps.LatLng(chosen.lat, chosen.lng);
    const map = new sdk.maps.Map(mapElement.current, { center, level: 3 });
    const marker = new sdk.maps.Marker({ map, position: center, draggable: true });
  ...
  return <section id="regular-stop-coordinate-setup" className="mx-auto mb-4 max-w-6xl px-4"><div className="rounded-2xl border border-blue-200 bg-white p-4 dark:border-blue-800 dark:bg-gray-800">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[260px_1fr]">
      ...
      <div><p className="text-sm font-black">{selectedName}</p><div className="mt-2 flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void runSearch(); }} placeholder="아파트명 또는 도로명 검색" className="min-w-0 flex-1 rounded-xl border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900" /><button type="button" onClick={() => void runSearch()} disabled={busy} className="rounded-xl bg-brand-navy-900 px-4 py-2 text-sm font-black text-white disabled:opacity-50">{busy ? "검색 중…" : "검색"}</button></div>
        ...
        {chosen && <div className="mt-3"><div ref={mapElement} className="h-64 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700" /><p className="mt-2 text-xs font-bold text-gray-600 dark:text-gray-300">핀이 정확하지 않으면 지도에서 핀을 끌어 실제 탑승 위치로 옮기세요.</p><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><span className="font-mono text-[11px] text-gray-400">{chosen.lat.toFixed(6)}, {chosen.lng.toFixed(6)}</span><button type="button" onClick={() => void saveSelected()} disabled={saving} className="rounded-xl bg-brand-orange-500 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">{saving ? "저장 중…" : "이 위치 확인 후 저장"}</button></div></div>}
      </div>
    </div>
  </div></section>;
}
