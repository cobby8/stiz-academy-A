...
- 목표: 시트 1차 정리본 기준으로 정규 셔틀·학생·수강료 데이터를 사이트와 랠리즈에 안전하게 이관한다.
- 현재 단계: 정규 차량표 전체 삭제 이관을 금지하고 증분 동기화 코드로 변경, 정규 배차 화면에 시간순 통합 타임라인 추가.
- 운영 반영: 없음. 코드 변경과 테스트만 수행했다.
- 확인보류: 9월 차량표 시트 기준 사이트 누락/초과 행, 셔틀비 0원 일반탑승 후보, 랠리즈 대조.
...
## 작업 로그 (최근 10건)
- 2026-09-02: 정규 차량표 이관을 전체 삭제 방식에서 증분 동기화로 바꾸고 직접 찍은 좌표를 장소 별칭 기준으로 보존하도록 수정했다. 정규 배차 화면은 등원/하원 분리 대신 시간순 통합 타임라인을 먼저 보여준다. 정규 셔틀 회귀 24건·tsc·diff-check 통과, 운영 DB·시트·랠리즈 쓰기 없음.
