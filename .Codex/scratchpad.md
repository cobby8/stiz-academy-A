...
## 현재 작업
- 작업명: 방학특강 수강·청구 전환 준비 표시
- 상태: 구현 및 검증 완료, 커밋 대기
- 범위: 기존 반/프로그램 연결 정보 조회, 신청 항목별 전환 준비 상태 표시, 회귀 테스트
- 기준일: 2026-07-20

## 진행 현황표
| 항목 | 상태 | 메모 |
| --- | --- | --- |
| 연결 정보 조회 | 완료 | 관리자 API가 linkedProgramId, linkedClassId를 신청 항목과 함께 전달 |
| 전환 준비 표시 | 완료 | 승인 필요, 정규 반 연결 필요, 전환 준비, 연결 완료 상태 표시 |
| 회귀 테스트 | 완료 | 수강·청구 전환 준비 UI가 빠지지 않도록 테스트 보강 |
...
- `src/app/api/admin/seasonal/route.ts`: 신청 항목에 기존 프로그램/반 연결 정보를 포함.
- `tests/seasonal-admin-ux.test.mjs`: 셔틀 상세, 빠른 처리, 전환 준비 UI가 빠지지 않도록 회귀 테스트 보강.
