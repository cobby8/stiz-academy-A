...
test("방학특강 정원 미정 반은 사이트 안에서 승인과 배정을 계속 처리할 수 있다", () => {
  assert.doesNotMatch(adminRoute, /offering\.capacity === null\) throw new SeasonalError\("정원이 정해지지 않은 반/);
  assert.doesNotMatch(adminRoute, /nextStatus === "OPEN" && nextCapacity === null/);
  assert.match(adminRoute, /offering\.capacity !== null && occupied >= offering\.capacity/);
  assert.match(adminRoute, /offering\.capacity !== null && occupied > offering\.capacity/);
});

test("방학특강 반별 명단은 신청자가 선택한 요일을 기준으로 필터링한다", () => {
  assert.match(adminRoute, /unnest\(app\."selectedWeekdays"\)/);
  assert.match(adminRoute, /SELECTED_WEEKDAYS_SQL/);
  assert.match(adminRoute, /array_agg\(day_num ORDER BY day_num\)/);
});
