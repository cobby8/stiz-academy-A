...
function BillingCard({ item, referenceTime, onRequest }: { item: SerializedStaffClassBillingItem; referenceTime: number; onRequest: () => void }) {
  ...
  // 금액이 어긋난 청구는 숨기지 않고 보여 주되, 잘못된 금액으로 수납되지 않도록 버튼만 막습니다.
  return <article className={`rounded-2xl border bg-white p-4 dark:bg-gray-900 ${item.status === "UNPAID" ? "border-red-200 dark:border-red-900" : "border-gray-200 dark:border-gray-800"}`}><div className="flex justify-between gap-3"><div><h3 className="font-black dark:text-white">{item.studentName}</h3><p className="mt-0.5 text-sm text-gray-500">{item.title}</p></div><span className={`h-fit rounded-full px-2.5 py-1 text-xs font-black ${statusClass}`}>{status}</span></div><div className="mt-4 flex items-end justify-between gap-3"><p className="text-2xl font-black dark:text-white">{item.amount.toLocaleString("ko-KR")}원</p><p className={`text-xs font-bold ${overdue ? "text-red-700" : "text-gray-500"}`}>납부 기한 {new Intl.DateTimeFormat("ko-KR").format(new Date(item.dueDate))}</p></div>{item.amountMismatch && <p className="mt-2 text-xs font-bold text-amber-700 dark:text-amber-300">청구서에는 {item.invoiceAmount.toLocaleString("ko-KR")}원으로 되어 있습니다.</p>}{item.status === "UNPAID" && (item.confirmable ? <button type="button" onClick={onRequest} className="mt-4 min-h-12 w-full rounded-xl bg-[var(--brand-accent)] font-black text-[var(--brand-accent-contrast)]">납부 확인 요청</button> : <p className="mt-4 flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-800 dark:bg-amber-950 dark:text-amber-200"><span className="material-symbols-outlined shrink-0 text-lg" aria-hidden="true">warning</span>{item.blockReason}</p>)}{item.status === "PENDING_CONFIRMATION" && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-800 dark:bg-amber-950 dark:text-amber-200">요청이 접수되었습니다. 관리자가 확인하면 납부 완료로 변경됩니다.</p>}</article>;
}
