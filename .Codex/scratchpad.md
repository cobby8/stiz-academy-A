...
- 현재 단계: 재확인 SMS 명시 승인·해시 대조·중복/재발급 방지·결과 장부와 신청ID 원장 연결 구현. TypeScript·QA 통과, 실제 발송/운영 DB/배포 미검증.
...
## 확인보류
- 다음 개발: 반 변경 전용 어댑터는 실제 시트 다반/신규행/복귀행 정책 확인 필요. 입학 외부 증거·최초 청구·초대 직접 연결, 과거 Payment만 있는 차액 건 확인, 실제 DB/공급자 E2E 전 배포 보류.
